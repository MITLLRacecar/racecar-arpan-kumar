...
def update():
    ...
    if rc.controller.is_down(rc.controller.Button.RB) or not rc.controller.is_down(rc.controller.Button.LB):
        if forward_dist < DIST and rt:
            speed = speed_controller(forward_dist, 1)
            print(f"going forward speed {speed}")
        
        if back_dist < DIST and lt:
            speed = speed_controller(back_dist, -1)
            print(f"going backward speed {speed}")
...
def speed_controller(distance, direction) :
